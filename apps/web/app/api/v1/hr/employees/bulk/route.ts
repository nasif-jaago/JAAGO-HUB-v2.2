  ...
  return isNaN(num) ? fallback : num;
    ...
    const employeePayloads = employees.map((emp: any) => {
      ...
      return {
        ...
        bank_branch: emp.bankBranch || emp.bank_branch || null,
        bank_account_number: emp.bankAccountNumber || emp.bank_account_number || null,
        bank_routing_number: emp.bankRoutingNumber || emp.bank_routing_number || null,
        bank_swift_code: emp.bankSwiftCode || emp.bank_swift_code || null,
        nick_name: emp.nickName || emp.nick_name || null,
        nid: emp.nid || null,
        blood_group: emp.bloodGroup || emp.blood_group || null,
        birthday: sanitizeDate(emp.birthday),
        ...
        dependent_children: sanitizeNumber(emp.dependentChildren || emp.dependent_children, 0),
        father_name: emp.fatherName || emp.father_name || null,
        mother_name: emp.motherName || emp.mother_name || null,
        spouse_name: emp.spouseName || emp.spouse_name || null,
        children_names: Array.isArray(emp.childrenNames) ? emp.childrenNames : Array.isArray(emp.children_names) ? emp.children_names : [],

        // Tab 3: Payroll
        joining_date: sanitizeDate(emp.joiningDate || emp.joining_date),
        contract_end_date: sanitizeDate(emp.contractEndDate || emp.contract_end_date),
        wage_type: emp.wageType || emp.wage_type || 'Fixed',
        wage: sanitizeNumber(emp.wage, 0),
        salary_jul_dec: sanitizeNumber(emp.salaryJulDec || emp.salary_jul_dec, 0),
        salary_jan_jun: sanitizeNumber(emp.salaryJanJun || emp.salary_jan_jun, 0),
        ...
        pf_rate: sanitizeNumber(emp.pfRate || emp.pf_rate, 10),
        regular_salary: sanitizeNumber(emp.regularSalary || emp.regular_salary, 0),
        extra_hours: sanitizeNumber(emp.extraHours || emp.extra_hours, 0),
        extra_payment: sanitizeNumber(emp.extraPayment || emp.extra_payment, 0),
        calculation_value: emp.calculationValue || emp.calculation_value || '1.0x',
        temporary_salary: sanitizeNumber(emp.temporarySalary || emp.temporary_salary, 0),
        total_current_salary: sanitizeNumber(emp.totalCurrentSalary || emp.total_current_salary, 0),
        currency: emp.currency || 'BDT',
        adjustment_start_date: sanitizeDate(emp.adjustmentStartDate || emp.adjustment_start_date),
        adjustment_end_date: sanitizeDate(emp.adjustmentEndDate || emp.adjustment_end_date),
        ...
        insurance_monthly_premium: sanitizeNumber(emp.insuranceMonthlyPremium ?? emp.insurance_monthly_premium ?? 1500, 1500),
        ...
        custom_office_days_from: sanitizeDate(emp.customOfficeDaysFrom || emp.custom_office_days_from),
        custom_office_days_to: sanitizeDate(emp.customOfficeDaysTo || emp.custom_office_days_to),
        ...
        leave_policy: emp.leavePolicy || emp.leave_policy || 'Standard Full-time Employee Policy',
        casual_leave_allocated: sanitizeNumber(emp.casualLeaveAllocated ?? emp.casual_leave_allocated ?? 14, 14),
        casual_leave_used: sanitizeNumber(emp.casualLeaveUsed ?? emp.casual_leave_used ?? 0, 0),
        sick_leave_allocated: sanitizeNumber(emp.sickLeaveAllocated ?? emp.sick_leave_allocated ?? 10, 10),
        sick_leave_used: sanitizeNumber(emp.sickLeaveUsed ?? emp.sick_leave_used ?? 0, 0),
        earned_leave_allocated: sanitizeNumber(emp.earnedLeaveAllocated ?? emp.earned_leave_allocated ?? 15, 15),
        earned_leave_used: sanitizeNumber(emp.earnedLeaveUsed ?? emp.earned_leave_used ?? 0, 0),
        special_leave_allocated: sanitizeNumber(emp.specialLeaveAllocated ?? emp.special_leave_allocated ?? 5, 5),
        special_leave_used: sanitizeNumber(emp.specialLeaveUsed ?? emp.special_leave_used ?? 0, 0),
        ...
        attendance_grace_period_min: sanitizeNumber(emp.attendanceGracePeriodMin ?? emp.attendance_grace_period_min ?? 15, 15),

        is_user: Boolean(emp.isUser || emp.is_user),
        user_id: sanitizeUuid(emp.userId || emp.user_id),
        updated_at: new Date().toISOString(),
      };
    });

    // ── 4. CHUNKED UPSERTS INTO SUPABASE WITH ROW-LEVEL FALLBACK ──
    ...
    for (let i = 0; i < employeePayloads.length; i += CHUNK_SIZE) {
      ...
      if (error) {
        console.warn(`Chunk ${Math.floor(i / CHUNK_SIZE) + 1} batch failed (${error.message}). Attempting row-by-row fallback...`);
        for (const row of chunk) {
          const { error: rowErr } = await supabaseAdmin
            .from('employees')
            .upsert(row, { onConflict: 'code' });
          if (rowErr) {
            console.error(`Row ${row.code} (${row.name}) upsert error:`, rowErr.message);
            errors.push(`Employee ${row.code} (${row.name}): ${rowErr.message}`);
          } else {
            totalUpserted++;
          }
        }
