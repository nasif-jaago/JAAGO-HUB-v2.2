...
async function syncNasif() {
  // Delete older code
  await supabaseAdmin.from('employees').delete().eq('code', 'EMP-NASIF-001');
  ...
  const { data, error } = await supabaseAdmin
    .from('employees')
    .upsert(nasifProfile, { onConflict: 'code' })
    .select();

  if (error) {
    console.error('Error syncing Nasif:', error);
  } else {
    console.log('✓ Successfully synced Nasif Kamal with code FO032507061190 into Supabase:', data);
  }
}

syncNasif().catch(console.error);
